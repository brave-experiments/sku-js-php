...
var cart_details = JSON.parse(JSON.stringify(original_details));
...
function replaceSkuTotalWithBATButton() {
  ...
  if (total) {
    ...
    total.innerHTML = '';
    button.onclick =  function () { showPaymentUI(cart_details); };
...
function addItemToPaymentRequest(details, sku) {
  if (Object.keys(details).length == 0) {
    details = JSON.parse(JSON.stringify(original_details));
